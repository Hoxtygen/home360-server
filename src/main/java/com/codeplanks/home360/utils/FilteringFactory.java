...
public class FilteringFactory {
  ...
  public static <T> Filtering parseFromParams(List<String> filter, Class<T> typeParameterClass) {
    ...
    for (String filterString : filter) {
      ...
      try {
        ...
        // if the operator is "in" or "nin", we need to split the value by ; to get the list of
        // values
  ...
  // this method uses recursion to get the last primitive type in the nested keys by travelling
  // down the object tree
  // this recursion method is safe because in the worst case, the number of recursive calls is
  // equal to the number of nested keys
  // in contradiction to the popular belief, recursion is not always bad and when you need to
  // travel down an object tree, it is the best solution
  private static Object nestedObject(
          Class classParameter, String value,
          String[] nested) throws NoSuchFieldException,
    ...
    if (nested.length > 1) {
      ...
      // we do this by calling this method again by removing the first nested key and passing the
      // type of the field
    ...
    // not only have we reached the last nested key, but we also have the type of the field
    // using the type of the field, we can get the correct converter from the map and convert the
    // value
